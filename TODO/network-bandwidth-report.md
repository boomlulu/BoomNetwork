...
| 优先级 | 优化项 | 预期效果 | 实现难度 |
|--------|--------|----------|----------|
| P0 | **轻量状态同步** (已实现): 帧同步停止时用状态消息+数据消息通信 | 非战斗阶段带宽→按需 | 中 |
| P1 | **输入 Delta 编码**: 重复输入只发 1 bit "same" | 减少 30-50% 数据量 | 中 |
| P2 | **大房间 AOI 分区**: 只广播可见玩家输入 | 大房间带宽降 80%+ | 高 |

> **关于空帧降频/抑制**: 不适用于实时游戏。BoomNetwork 的物理/AI/buff 等逻辑依赖逐帧
> `OnFrame` 推进，空帧降频会导致画面冻结+跳帧。空帧带宽（180 B/s/客户端）本身占比
> 极小（<6%），不是真正的瓶颈。正确的做法是在帧同步未运行的阶段（选人/等待/大厅）
> 使用 P0 的轻量状态同步代替。

#### P0: 轻量状态同步（帧同步的互补方案）

帧同步运行时保持 20fps 逐帧推送不变。帧同步未运行的阶段（选人、等待、大厅、结算）
使用两种新消息类型通信:

- **状态消息 (StateMessage)**: 服务器纯转发，不存储。用于一次性事件（选人完成、准备就绪、聊天）
- **数据消息 (DataMessage)**: 键值对存储在服务器，增量广播。用于持续状态（角色选择、准备状态、分数）

两种模式各司其职:
```
战斗中:  帧同步 20fps — 高频、确定性、逐帧推进
非战斗:  状态同步 按需 — 低频、事件驱动、增量更新
```

**实现状态**: 已完成 (ExtCmd 50-55)，详见 [state-sync-protocol.md](state-sync-protocol.md)
